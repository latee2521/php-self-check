...
if (!empty($_SESSION['patron_barcode']) && !empty($_SESSION['patron_password'])){
	if (empty($_POST['barcode']) && (!is_numeric($_POST['barcode']) || strlen($_POST['barcode']) != 14)){ //if for some reason the item barcode posted is empty fill in a bunk one
		$item_barcode = 'bunk_barcode';
		
	} else {
		$item_barcode = trim($_POST['barcode']);
	}
	...
	} elseif (empty($_SESSION['cko_barcodes']) OR !in_array($item_barcode,$_SESSION['cko_barcodes'])){
	...
	if (!empty($checkout['variable']['AJ'][0])){
			...
			$title = ucwords(TrimByLength($title, 50, false));
	}
	...
	if(stripos($response_message, $already_ckdout_to_you) === false){
